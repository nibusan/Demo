...
Quaternion::Quaternion(const Vector3<double>& rad) {
...
Quaternion Quaternion::Euler(const Vector3<double>& rad) {
...
Quaternion Quaternion::AngleAxis(double rad, Vector3<double> axis) {
...
Vector3<double> Quaternion::PosAxis(const Quaternion& q, Vector3<double> pos) {
...
Vector3<double> Quaternion::PosAxis(Vector3<double> pos) const {
	return PosAxis(Quaternion(w, x, y, z), pos);
}

Vector3<double> Quaternion::ToEuler(const Quaternion& q) {
...
Vector3<double> Quaternion::ToEuler(void) const {
...
Quaternion Quaternion::LookRotation(Vector3<double> dir) {
	Vector3<double> up = { 0.0f, 1.0f, 0.0f };
	return LookRotation(dir, up);
}

Quaternion Quaternion::LookRotation(Vector3<double> dir, Vector3<double> up) {

	dir = dir.Normalize();
	Vector3<double> right = (up.Cross(dir)).Normalize();
	up = dir.Cross(right);
	auto m00 = right.x;
	auto m01 = right.y;
	auto m02 = right.z;
...
Vector3<double> Quaternion::GetDir(Vector3<double> dir) const {
	return PosAxis(dir);
}

Vector3<double> Quaternion::GetForward(void) const {
	return GetDir(Utility::DIR_F);
}

Vector3<double> Quaternion::GetBack(void) const {
	return GetDir(Utility::DIR_B);
}

Vector3<double> Quaternion::GetRight(void) const {
	return GetDir(Utility::DIR_R);
}

Vector3<double> Quaternion::GetLeft(void) const {
	return GetDir(Utility::DIR_L);
}

Vector3<double> Quaternion::GetUp(void) const {
	return GetDir(Utility::DIR_U);
}

Vector3<double> Quaternion::GetDown(void) const {
...
Quaternion Quaternion::FromToRotation(Vector3<double> fromDir, Vector3<double> toDir) {

	Vector3<double> axis = fromDir.Cross(toDir);
	double angle = Utility::AngleDeg(fromDir, toDir);
	if (angle >= 179.9196) {
		auto r = fromDir.Cross(Utility::DIR_R);
...
Quaternion Quaternion::SlerpUnclamped(Quaternion a, Quaternion b, float t) {
	...
	float cosHalfAngle = (float)(a.w * b.w) + a.xyz().Dot(b.xyz());
	...
	Vector3<double> v = a.xyz() * blendA + b.xyz() * blendB;
...
Vector3<double> Quaternion::xyz(void) const {
	return { x, y, z };
}

void Quaternion::ToAngleAxis(float* angle, Vector3<double>* axis) {
