...
void ResourceManager::LoadSceneResourceFile(Scene::TYPE sceneType) {
	// 現在のシーンの種類を取得
	const auto currentSceneType = SceneManager::GetInstance().GetCurrentSceneType();
...
void ResourceManager::RegisterUsingResourceList(const nlohmann::json& resourceFileListJson) {
	// 各シーンの使用するリソースファイルを登録したリストを読み込む
	for (int i = 0; i < static_cast<int>(Scene::TYPE::MAX); i++) {
